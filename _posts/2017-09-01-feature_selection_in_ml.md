...
在过滤式和包裹式特征选择方法中，特征选择过程与学习器训练过程有明显的分别；与此不同，嵌入式特征选择是特征选择过程与学习器训练过程融为一体，两者在同一个优化过程中完成，即在学习器训练过程中自动地进行了特征选择。

给定数据集 $$D=\{ (x_1, y_1), (x_2, y_2), \dots, (x_m, y_m) \}$$，其中 $${x} \in \mathbb{R} ,y \in \mathbb{R}$$。考虑最简单的线性模型回归模型，以平方误差为损失函数，则优化目标为：

$$\min_{w} \sum_{i=1}^m {(y_i - w^T x_i)^2}$$

当样本特征很多，而样本数相对较少时，上式很容易陷入过拟合。为了缓解过拟合问题，可对上式引入正则化想。若使用 $${\rm L_2}$$ 范数正则化，则有：

$$\min_{w} \sum_{i=1}^m {(y_i - w^T x_i)^2 + \lambda \| w \|_2^2}$$

其中正则化参数 $$\lambda > 0$$。上式也称为“岭回归” (ridge regression) [Tikhonov and Arsenin, 1977]，通过引入 $$\rm L_2$$ 范数正则化，确能显著降低过拟合的风险。

那么，能否将正则化项中的 $$\rm L_2$$ 范数替换为 $$\rm L_p$$ 范数呢？答案是肯定的。若令 $$p=1$$，即采用 $$\rm L_1$$ 范数，则有：

$$\min_{w} \sum_{i=1}^m {(y_i - w^T x_i)^2 + \lambda \| w \|_1}$$

其中正则化参数 $$\lambda > 0$$。上式称为 LASSO (Least Absolute Shrinkage and Selection Operator) [Tibshirani, 1996]。

 $$\rm L_1$$ 范数和 $$\rm L_2$$ 范数正则化都有助于降低过拟合风险，但 $$\rm L_1$$ 范数还会带来一个额外的好处：它比 $$\rm L_2$$ 范数更易于获得“稀疏” (sparse) 解，即它求得的 $$w$$ 会有更少的非零分量。

注意到 $$w$$ 取得稀疏解意味着初始的 $$d$$ 个特征中仅有对应着 $$w$$ 的非零分量的特征才会出现在最终模型中，于是，求解 $$\rm L_1$$ 范数正则化的结果是得到了仅采用一部分初始特征的模型；换言之，基于 $$\rm L_1$$ 正则化的学习方法就是一种嵌入式特征选择方法，其特征选择过程与学习器训练过程融为一体，同时完成。